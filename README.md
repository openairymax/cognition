...
> Cognition subsystem of [AirymaxOS](https://atomgit.com/openairymax/agentrt-linux) — the AI Agent Operating System.
...
- **Hyper-node sandbox** delegating agent isolation to the AirymaxOS Security layer

### Relationship with Airymax coreloopthree + frameworks

The AirymaxOS Cognition reuses and extends the `coreloopthree` and `frameworks` modules from the Airymax runtime platform. The three-loop cognition model, agent framework abstractions and Wasm runtime are shared between the user-space runtime (agentrt) and the OS-level cognition engine (AirymaxOS), ensuring architectural homology with no adaptation layer.
...
- **AirymaxOS Kernel** — provides the kthread, sched_ext and io_uring primitives for the cognition loop
- **AirymaxOS Memory** — provides the tiered CXL/PMEM memory used for LLM serving and KV-cache
- **Airymax coreloopthree + frameworks** — provides the three-loop model and agent frameworks that are reused and extended

### Downstream

- **AirymaxOS Services** — service layer that exposes cognition capabilities to user space
